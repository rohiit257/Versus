...
export default function NavbarClient({ user }: NavbarClientProps) {
  ...
  return (
    <motion.nav
      ...
      className="relative z-50 w-full border-b border-border bg-background/90 backdrop-blur-sm"
    >
              ...
              <div className="hidden md:flex items-center space-x-6">
                ...
                <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                  <button 
                    ...
                    className="group relative flex h-10 w-10 items-center justify-center rounded-lg bg-muted/50 text-foreground transition-all duration-300 hover:bg-emerald-500/20 hover:text-emerald-400"
                  >
                    <Plus size={20} />
                    <Tooltip text="Create" />
                  </button>
                ...
                <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                  <button className="group relative flex h-10 w-10 items-center justify-center rounded-lg bg-muted/50 text-foreground transition-all duration-300 hover:bg-emerald-500/20 hover:text-emerald-400">
                    <Bell size={20} />
      ...
      <Link
        ...
        className={`group relative flex h-10 w-10 items-center justify-center rounded-lg transition-all duration-300 ${
          isActive 
            ...
            : "bg-muted/50 text-foreground hover:bg-emerald-500/20 hover:text-emerald-400"
        }`}
      >
        <Icon size={20} />
...
function Tooltip({ text }: { text: string }) {
  return (
    <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      <div className="bg-background text-foreground text-xs px-2 py-1 rounded whitespace-nowrap border border-border">
        {text}
      </div>
    </div>
  );
